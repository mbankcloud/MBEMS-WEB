...
export default async function DirectorPollingPage() {
  ...
  const { data: agents } = await db.from("agents").select("id, agent_code").eq("director_id", director.id)
  const agentIds = agents?.map(a => a.id) ?? []

  const { data: records } = agentIds.length > 0
    ? await db.from("polling_records")
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold">Polling Overview</h1>
